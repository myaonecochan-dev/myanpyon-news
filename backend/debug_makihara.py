...
def debug_makihara():
    print("Checking Makihara post...")
    res = supabase.table("posts").select("*").ilike("title", "%牧原%").order("created_at", desc=True).limit(1).execute()
    ...
    if not val_myan or str(val_myan) == "None":
         print("ALERT: Comments are MISSING! Myan is None.")
         # We do NOT backfill here, we want to see if the collector did its job.
    else:
         print("SUCCESS: Comments exist! Persistent Retry Logic worked.")
