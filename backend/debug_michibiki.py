...
def debug_michibiki():
    print("Checking Michibiki post...")
    res = supabase.table("posts").select("*").ilike("title", "%みちびき%").execute()
    ...
    if not val_myan:
         ...
         update_data = {
            "comment_myan": "税金が空の彼方に消えたにゃ…H3ロケット、がんばってくれよ～！",
            "comment_pyon": "失敗は成功の母といいますが、コストを考えると笑えませんわね。原因究明を待ちましょう。"
         }
